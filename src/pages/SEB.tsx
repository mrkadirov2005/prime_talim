...
export default function SEB() {
  ...
  useEffect(() => {
    if (isRegistered) {
      ...
      if (mainRef.current) mainRef.current.style.display = "";
    }
  }, [isRegistered]);
  ...
  const handleClick = (index: number, chosenOption: string, correctAnswer: string) => {
    ...
    setCorrects((prev) => {
      if (chosenOption === correctAnswer) {
        return prev + 1;
      } else if (selectedAnswers[index] === correctAnswer) {
        return prev - 1;
      }
      return prev;
    });
  ...
  const RenderTests = ({ tests }: { tests: ITEM[] }) => {
    return (
      <section className="w-full max-w-2xl mx-auto p-4">
        ...
        {showResults && (
          <div className="mt-6 text-center text-white text-xl font-bold">
            ...
            <a href="/" className="mt-4 !rounded-md bg-blue-500 p-2">
              Asosiyga qaytish
            </a>
          </div>
        )}
